...
function AddAdvanceDialog({ onClose }: { onClose: () => void }) {
  ...
  const bankOnly = (accounts ?? []).filter((a) => a.account_type !== "cash");
  ...
  async function submit() {
    ...
    await create.mutateAsync({
      vendor_name: vendor, category, total_amount: amt, paid_date: paidDate,
      ...
      <DialogContent className="md:!max-w-md">
        ...
        <div className="space-y-3">
          <FormField label="Vendor" required htmlFor="pa_vendor">
            <Input id="pa_vendor" placeholder="e.g. Facebook / Google Ads" value={vendor} onChange={(e) => setVendor(e.target.value)} autoFocus />
          </FormField>
          ...
          {bankOnly.length > 0 && (
            <FormField label="From which account?" htmlFor="pa_bank">
              <Select value={bankId || "none"} onValueChange={(v) => setBankId(v === "none" ? "" : v)}>
                <SelectTrigger id="pa_bank"><SelectValue placeholder="Select bank account" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Not sure / pick later</SelectItem>
                  {bankOnly.map((a) => <SelectItem key={a.id} value={a.id}>{a.name}{a.bank_name ? ` · ${a.bank_name}` : ""}</SelectItem>)}
                </SelectContent>
              </Select>
              <p className="text-[10px] text-ink-3 mt-1">Banking me isi account ki debit line se reconcile karo.</p>
            </FormField>
          )}
