...
export function useAttendanceNetwork() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ allowedIps: string[]; currentIp: string; onAllowedNetwork: boolean; requireSelfie: boolean; requirePresence: boolean; retentionDays: number }>;
    },
    staleTime: 10_000,
  });
}
...
export function useSetAttendanceNetwork() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (input: { action: "lock" | "clear" | "remove" | "require_selfie" | "require_presence" | "set_retention"; ip?: string; value?: boolean | number }) => {
      const res = await fetch("/api/attendance/network", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
