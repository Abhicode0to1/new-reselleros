-- HOTFIX for 20260822140000, which broke record_payment for EVERY payment.
--
-- That migration defaulted v_extension_months to the term of the subscription being
-- renewed. Correct intent, wrong place: the assignment sits ~9 lines ABOVE the select that
-- fills v_renewal_sub, and in plpgsql a not-yet-assigned RECORD has an indeterminate tuple
-- structure — reading a field raises 55000. The line runs unconditionally, so it took down
-- new sales as well as renewals.
--
-- Found within a minute because the SQL regression test runs record_payment for real
-- rather than inspecting it. A test that asserted on the function's TEXT would have been
-- perfectly happy.
--
-- The default now resolves immediately after v_renewal_sub is selected, and only when the
-- quote itself did not state an extension. New sales keep the old constant of 12, which is
