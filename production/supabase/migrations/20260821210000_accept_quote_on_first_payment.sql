-- Accept a quote on the FIRST payment, not only on full payment.
--
-- WHY: record_payment already did the irreversible half of "accepted" on the first
-- payment of ANY size — it inserts the customer row (lead -> customer) and creates the
-- subscription. Only the status lagged behind, gated on v_is_fully_paid. So a quote that
-- had taken Rs 50,000 of Rs 50,027 sat at 'sent', the lifecycle bar read "Waiting for the
-- customer to accept", and the page offered a "Mark accepted" button whose stated purpose
-- -- "convert the lead into a customer" -- had already happened. Somebody who has paid has
-- accepted; the money is the fact and the status was the lagging label.
--
-- Found by Pardeep on Q-ADPL-2026-27-0026 (delhom), 21 Aug 2026.
--
-- SCOPE: exactly one line of the function body changes, and this file was generated from
-- the live definition rather than retyped, with an assertion that only that line differs.
--
--   - when v_is_fully_paid and status in ('draft','sent','viewed') then 'accepted'
--   + when status in ('draft','sent','viewed') then 'accepted'
--
-- Everything else is byte-identical to what production runs today.
--
-- WHAT IS DELIBERATELY NOT CHANGED:
--
--  * delete_payment already reverses this correctly and needs no edit. It sets
--    status = 'sent' when v_remaining <= 0 and status = 'accepted', so deleting the
--    payment that caused the promotion demotes the quote again. Checked before writing
--    any DDL for it.
--  * The renewal roll-forward stays gated on full payment (v_is_renewal_quote and
--    v_is_fully_paid). Rolling a subscription forward a whole year on a part payment
--    would be giving away service, which is a different decision from labelling a quote.
--  * payment_status keeps its three values -- 'partial' still means partial. This changes
--    the WORKFLOW status only; the money columns are untouched.
--
-- SAFE BY THE FUNCTION'S OWN GUARDS: nothing reaches this line without real money.
-- p_amount <= 0 raises, and a duplicate reference returns early well before the UPDATE.
--
