-- Monthly subscriptions become renewable: a real renewal date, a term-aware roll-forward,
-- and a reminder ladder that suits a 30-day cycle.
--
-- FOLLOWS 20260822120000, which made monthly sales produce a subscription at all but left
-- renewal_date NULL. That was the right call at the time and the wrong end state: NULL kept
-- them out of a cadence that would have emailed a monthly customer seven times a month,
-- but it also meant nothing ever chased them. lib/renewals/cadence.ts now picks the ladder
-- from the TERM (T-7/T-3/T-0 for a one-month term, the usual seven for anything longer),
-- so the honest date is finally safe to store.
--
-- ─── THE DISTINCTION THAT SHAPED THIS, AND IT CAME FROM PARDEEP ─────────────
-- "monthly renewal do cases me ho sakta hai — ek monthly commitment monthly payment,
--  doosra annual commitment monthly payment."
--
-- Both are real, and this codebase already separates them (migration 0161): `commitment`
-- is the PRICE TIER, `quotes.billing_cycle` is the INVOICE FREQUENCY.
--
--   flex monthly          commitment 'monthly',       term 1   → renews every month
--   annual, paid monthly  commitment 'annual_yearly', term 12  → renews once a year
--
-- The second is an ordinary Workspace arrangement. My first cut keyed the reminder ladder
-- off billing_cycle, which would have handed that customer three reminders for the end of
-- a YEAR-long commitment — no runway to raise a PO. The ladder now keys off term_months.
-- record_payment was already correct here: it gates on `commitment`, so an annual plan
-- paid monthly still takes the annual path, +1 year, MRR/12.
--
-- ─── THE ROLL-FORWARD WAS THE QUIETER BUG ───────────────────────────────────
-- v_extension_months defaulted to a hard 12. Renewing a MONTHLY subscription therefore
-- pushed its renewal date twelve months out: never chased again, and its MRR counted for a
-- year of service nobody had sold. It now defaults to the term of the subscription being
-- renewed.
--
-- And 'renewed' was only set when the extension reached 12 months, so a monthly renewal
-- left renewal_state stuck wherever the cadence had got to — the ladder never re-armed and
-- the following month went unchased. Now: a full TERM counts as renewed.
--
-- SCOPE: five anchored edits, generated from the live definition, each asserted to match
-- exactly once.
...
begin
  ...
  v_extension_months     := coalesce(v_quote.extension_months, v_renewal_sub.term_months, 12);
