...
- [~] **Brick #5: Customer-facing site → ResellerOS** (2 Sep) FOUNDATION done: website (35 file) src/site me vendored + imports repoint; CSS .anutech-site me scoped (design isolation browser-verified); (marketing) route group + nested layout (Archivo/Plex + chrome); /hosting + /domains + 3 API routes port; build + 6353 test green; route-guards scoped. BAAKI: free pages (/ssl,/email,/reseller,/cart,/checkout,...) mechanical; colliding (/,/pricing,/about,/login,...) "kaun jeete" decision; engine public pages retire.
